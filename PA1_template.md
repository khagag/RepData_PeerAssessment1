...
eachDayCounts <- c()
for (dt in dates) {
    eachDayCounts <- c(eachDayCounts,sum(as.numeric(activities[activities$date == dt,"steps"])));
}
```
...
```r
the_mean <- mean(eachDayCounts);
the_median <- median(eachDayCounts);
mean(eachDayCounts)
```
...
```r
median(eachDayCounts)
```

```
## [1] 10765
```
The mean is 1.0766189\times 10^{4} & The median is 1.0765\times 10^{4}

## What is the average daily activity pattern?

```r
stepsInterval <- aggregate(steps ~ interval,data = activities ,mean)
